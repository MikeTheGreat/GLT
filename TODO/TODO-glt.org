...
** FEATURE: in .gltrc, define a pattern to indicate where to download .git dir (ex: <class>/<assign>/<username.last>, <username.first>, <assign>)
...
** FEATURE: list all homeworks
** FEATURE: list all students in a class
** FEATURE: list all config options, and where they came from?
** Verify GitLab setup
...
** NEED: tutorial for setting up the student side of things
...
** Go through this file, clean it up & figure out next steps
** Upload feedback
...
** NEED: GitDo needs to gracefully handle non-existent commands (e.g., missing .bat fil)
